...
> They are all hash functions because they take an input and produce a value, which is used to map to an index in a hash table. They share 2 key properties: "key" and "size". The "key" is used to produce a unique value for mapping and retrieving data, and the "size" ensures that the index produced is always within the size of the hash table to avoid out of bound errors. 
...
> I would choose pearson_hash function. Based on the analysis in question 2, while sha256 offers the best collision resistance and the highest security, for a TAFE assignment where data integrity and security are not a main concern, the Pearson hash function is more suitable. It has good uniformity, determinism and efficient. Although its collision resistance may be weak, this can be managed with collision resolutions.

5. In your own words, explain each line in the pearson hash function above in terms of the criteria you listed in question 2.

>  import random (using randomness which can create a more even distribution-uniformity)
   random.seed(42)  (choosing a start point, ensuring the same random numbers are generated-determinism)
   pearson_table = list(range(256)) (using a fixed range produces a more predictable behaviour-determinism)
